...
export class RestaurantsService {
	...
	async getByLocation(query: Record<string, string | undefined>) {
		...
		let restaurants = await this.restaurantModel.findAll({
			where,
		...
		if (!restaurants.length) {
			...
			restaurants = await this.restaurantModel.findAll({
				where: fallbackWhere,
				include: this.getLocationInclude(),
			});
		}
		...
		return {
			nearby_restaurants: restaurants.map((restaurant) => this.toLocationResponse(restaurant)),
		};
	}

	private toLocationResponse(record: TempleNearbyRestaurant): Record<string, unknown> {
