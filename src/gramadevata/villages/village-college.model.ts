...
import { EntityStatus, SchoolType } from '../../common/enums';
// import { Register } from '../auth/user.model';

@Table({ tableName: 'village_college', timestamps: false })
export class VillageCollege extends Model<VillageCollege> {
  @Column({
    type: DataType.UUID,
    allowNull: false,
    primaryKey: true,
  ...
  @Column({ type: DataType.STRING(45), allowNull: true, defaultValue: SchoolType.PRIVATE, field: 'college_type', validate: { isIn: [Object.values(SchoolType)] } })
  declare collegeType?: string;
